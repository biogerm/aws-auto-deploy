...
current="5"
while true ; do
   volumes=`ssh root@localhost -p 7022 ls -a /Volumes`
   number_of_volumes=`ssh root@localhost -p 7022 ls -lah /Volumes | wc -l`
   ...
   if [ $number_of_volumes -eq $current ] ; then
      ...
      if [ $power_state == "off" ] ; then
        ...
        echo `date +%Y%m%d' '%H:%M:%S`" Volume: [ "$volumes" ]" | mail -s "Power on detected" qin.an@ericsson.com
        power_state="on"
      fi
   else
      if [ $number_of_volumes -eq "0" ] ; then
        ...
        if [ $power_state == "on" ] ; then
          ...
          echo `date +%Y%m%d' '%H:%M:%S` | mail -s "Power off detected" qin.an@ericsson.com
          power_state="off"
        fi
      else
        echo `date +%Y%m%d' '%H:%M:%S`" Volume changed: [ "$volumes" ]"
        echo "Volume list: " $volumes | mail -s "Strange file found" qin.an@ericsson.com
